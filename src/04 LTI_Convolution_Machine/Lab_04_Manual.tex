...
\large{\bf Hardware-in-the-Loop Discrete-Time Convolution}
\end{center}
...
\delECE{This is a graded laboratory assignment. Include your \textbf{name and student ID} in all required screenshots and photographs. Work submitted without identifying information may receive reduced credit.}

\delECE{All MATLAB verification must use data captured from your own hardware setup. Convolution results computed entirely in MATLAB without physical Arduino acquisition are \emph{not} accepted as hardware verification evidence.}

\cpECE{Before submission, verify that all screenshots are legible, include properly labeled axes, and display your identifying information where applicable.}
...
\begin{itemize}
    ...
    \item \color{OliveGreen}\textbf{Checkpoint ($N$): }Evidence to show your TA during lab for in-session sign-off.\color{black}
    \item \color{Plum}\textbf{Discussion ($N$): }Analysis questions to answer in your post-lab submission.\color{black}
...
This laboratory transforms the abstract mathematics of discrete-time (DT) convolution into a physical, observable machine. You will build a hardware system called the \emph{Convolution Machine}: an Arduino-based circuit that reads a finite-duration impulse response $h[n]$ from a bank of DIP switches, accepts an input sequence $x[n]$ through momentary pushbuttons, computes the full convolution sum
\[
    y[n] = x[n] * h[n] = \sum_{k=-\infty}^{\infty} x[k]\, h[n - k]
\]
in firmware, and displays the output $y[n]$ on a 10-segment LED bar graph. Simultaneously, the Arduino streams all three sequences over a USB-UART serial link to MATLAB, where you will verify the hardware result using the built-in \texttt{conv()} function, produce publication-quality stem plots, and animate the ``flip--shift--multiply--sum'' interpretation of convolution frame by frame.

By the end of this lab, the convolution sum will no longer be a formula you manipulate symbolically; it will be a process you have wired, implemented, debugged, and watched.
...
\begin{enumerate}
    \item \textbf{Compute DT convolution by hand} using the flip--shift--multiply--sum procedure, explicitly tracking each output sample $y[n_0]$ for every index $n_0$ in the output support.
    \item \textbf{Implement the convolution sum in embedded C++} on an Arduino, reading hardware inputs (DIP switches, pushbuttons) and driving hardware outputs (LED bar graph) to form a closed-loop real-time system.
    \item \textbf{Verify hardware convolution results} with MATLAB's \texttt{conv()} function, identify potential sources of discrepancy, and quantify agreement between physical and software output.
    \item \textbf{Construct and interpret stem plots} of $x[n]$, $h[n]$, and $y[n]$ in a properly formatted MATLAB figure with labeled axes and correct discrete-time index ranges.
    \item \textbf{Animate the flip--shift--multiply--sum process} in MATLAB to build an intuitive graphical understanding of how the flipped kernel $h[-k]$ sweeps across the input $x[k]$ to produce each successive output sample.
\end{enumerate}
